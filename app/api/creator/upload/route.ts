 ...
 *
 * Authenticated creator file upload into GPM 4PE-MSC.
 * Requires a valid gpmc_creator_session cookie.
 * Auto-tags every asset with: { brand, creator, type, scope, uploaded }.
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Validate scope belongs to this creator
    ...
    // Validate file MIME type for this scope
    ...
    // Build storage path: creator-assets/{BRAND}/{scope}/{timestamp}-{filename}
    ...
    // Load creator_portals row to get the canonical id
    ...
    const { data: asset, error: dbError } = await supabaseAdmin
      ...
      .insert({
        ...
        brand:           creator.brand,
        creator_auth_id: session.portalId,  // JWT portalId used as identity ref
        file_url:        publicUrl,
