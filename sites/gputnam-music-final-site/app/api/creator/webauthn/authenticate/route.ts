 ...
 * Two-phase WebAuthn authentication for GPM creator portal.
 */
  ...
  try {
    ...
    if (!portal.webauthn_credential_id) {
      return NextResponse.json(
        { error: 'Creator not yet enrolled — visit /creator/enroll first' },
        { status: 412 }
      );
    }
    ...
    if (phase === 'options') {
      ...
      const options = await generateAuthenticationOptions({
        ...
        allowCredentials: [{ id: portal.webauthn_credential_id as string }],
      });
