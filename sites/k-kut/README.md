...
## Audio Player (`KutAudioPlayer`)

### How it works

All audio playback on k-kut.com is handled by a single persistent `KutAudioPlayer` component mounted in the root layout. It renders as a fixed bar at the bottom of every page and appears as soon as any track is triggered.

**One audio source at a time** — no simultaneous playback conflicts.

### Event API

Any component can trigger playback by dispatching a `kkut-play` CustomEvent:

```typescript
window.dispatchEvent(new CustomEvent('kkut-play', {
  detail: {
    url:          'https://your-supabase-project.supabase.co/storage/v1/object/public/tracks/your-file.mp3',
    title:        'Love Renews',
    artist:       'KLEIGH',
    type:         'K-KUT',       // 'K-KUT' | 'mK' | 'KPD'
    romanceLevel: 'Love',        // KPD only — Interest | Date | Love | Sex | Forever
  },
}));
```

To pause the current track from any component:
```typescript
window.dispatchEvent(new CustomEvent('kkut-pause-request'));
```

To listen for state changes (e.g. animate a waveform):
```typescript
window.addEventListener('kkut-playing', (e) => {
  const { url } = (e as CustomEvent<{ url: string }>).detail;
  // url = the track that just started playing
});
window.addEventListener('kkut-paused', (e) => { /* ... */ });
window.addEventListener('kkut-ended',  (e) => { /* ... */ });
```

### Supabase MP3 URL configuration

The `url` field should be a **direct MP3 URL** — either:
1. **Public bucket URL** (for non-gated preview tracks):
   ```
   https://<project-ref>.supabase.co/storage/v1/object/public/tracks/<file>.mp3
   ```
2. **Signed URL** (for paid/gated tracks — generated by an edge function):
   ```
   https://<project-ref>.supabase.co/storage/v1/object/sign/tracks/<file>.mp3?token=...
   ```

The `tracks` table row should have a `url` column containing one of the above. The three
invention fetchers (`getFeaturedKuts`, `getFeaturedMKuts`, `getFeaturedKPDs`) in
`lib/featuredKuts.ts` already `select('url')` and forward it to the player.

### Player UI colours per invention

| Invention | Accent |
|-----------|--------|
| K-KUT (KK) | Amber `#FBBF24` |
| mini-KUT (mK) | Rich brown `#8B5E3C` / tan `#C8A882` |
| K-kUpId (KPD) | Rose `#FB7185` |
